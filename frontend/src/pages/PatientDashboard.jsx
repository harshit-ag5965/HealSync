      ...
      <aside className={`${sidebarOpen ? "w-64" : "w-20"} transition-all duration-300 flex-shrink-0 ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} border-r flex flex-col shadow-sm`}>
        <div className={`p-5 border-b ${darkMode ? "border-gray-700" : "border-gray-100"} flex items-center justify-between`}>
          <div className={`flex items-center gap-3 ${!sidebarOpen && "justify-center"}`}>
            <div className="w-9 h-9 bg-gradient-to-br from-blue-500 to-blue-700 rounded-xl flex items-center justify-center flex-shrink-0 shadow">
              <span className="text-white text-lg">🏥</span>
            </div>
            {sidebarOpen && (
              <div>
                <p className={`font-black text-base ${darkMode ? "text-white" : "text-blue-700"}`}>HMS</p>
                <p className="text-xs text-gray-400">Patient Portal</p>
              </div>
            )}
          </div>
          <button onClick={() => setSidebarOpen(!sidebarOpen)} className={`text-gray-400 hover:text-gray-600 text-lg ${!sidebarOpen && "hidden"}`}>◀</button>
        </div>
